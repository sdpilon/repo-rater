...
test("loadRun does not crash and does not record a fetch_failures row when extract failed before a repo_id was known", async () => {
  ...
  const bronzeDir = fs.mkdtempSync(path.join(os.tmpdir(), "bronze-"));
  const extractResults = [
    { fullName: "sdpilon/broken-repo", repoId: null, dataType: "repo", status: "error", error: "totally unexpected failure" },
  ];
  const summary = await loadRun({ db, runId: "run_1", bronzeDir, extractResults, now: "2026-07-22T00:00:00.000Z" });
  assert.equal(summary.reposLoaded, 0);
  assert.equal(summary.failuresRecorded, 0);
  const failures = await db.all("SELECT COUNT(*)::INTEGER AS n FROM fetch_failures");
...
test("loadRun loads a successful data type and records a failure for a different data type on the same repo, independently", async () => {
  ...
  const extractResults = [
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "meta", status: "ok" },
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "commits", status: "ok", since: "2020-01-01T00:00:00Z", fetchedAt: "2026-07-22T00:00:00.000Z" },
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "issues", status: "error", error: "issues endpoint timed out" },
  ];
  const summary = await loadRun({ db, runId: "run_1", bronzeDir, extractResults, now: "2026-07-22T00:00:00.000Z" });
  assert.equal(summary.failuresRecorded, 1);
  const commits = await db.all("SELECT sha FROM commits WHERE repo_id = 1");
  assert.equal(commits.length, 1);
  const issues = await db.all("SELECT COUNT(*)::INTEGER AS n FROM issues WHERE repo_id = 1");
...
test("loadRun preserves first_ingested_run_id across repeated loads of the same commit", async () => {
  ...
  const extractResultsRun1 = [
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "meta", status: "ok" },
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "commits", status: "ok", since: "2020-01-01T00:00:00Z", fetchedAt: "2026-07-20T00:00:00.000Z" },
  ];
  await loadRun({ db, runId: "run_1", bronzeDir, extractResults: extractResultsRun1, now: "2026-07-20T00:00:00.000Z" });
  writeFixtureBronze(bronzeDir, "run_2", 1);
  const extractResultsRun2 = [
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "meta", status: "ok" },
    { fullName: "sdpilon/spilon.dev", repoId: 1, dataType: "commits", status: "ok", since: "2020-01-01T00:00:00Z", fetchedAt: "2026-07-22T00:00:00.000Z" },
  ];
  await loadRun({ db, runId: "run_2", bronzeDir, extractResults: extractResultsRun2, now: "2026-07-22T00:00:00.000Z" });
  const rows = await db.all("SELECT first_ingested_run_id FROM commits WHERE repo_id = 1 AND sha = 'aaa'");
