...
**1. The app** — the SolidStart dashboard, deployed via Vercel's native Git integration: pushing to `main` triggers Vercel's own build and deploy directly, for both the maintainer's production instance and a separate public fake-data demo project. `.github/workflows/ci.yml` is a parallel, deploy-independent gate — installs, typechecks, lints, and tests on every push to `main` and every PR, with no Vercel CLI step and no deploy action of its own. Locally: `pnpm dev` — plain `vite dev`, no secret-injection wrapper. Secrets (`DATABASE_URL`, `APP_PASSWORD`, etc.) resolve via the env-var-first, config-file-fallback resolver described in the README's "Self-hosting" section, not a local `.env` file. Any personal secret-injection wrapping (e.g. 1Password's `op run`) is the maintainer's own shell setup, not part of this repo.

**2. The pipeline (`src/pipeline/`)** — discovers every repo in the account via Octokit (`listForAuthenticatedUser`), then for each one: fetches readme/issues/prs/commits/meta, upserts into Postgres, and runs a content-hash-gated AI assessment (skipping repos marked ignored or with a manually-overridden assessment). There is no automated trigger — it's run on demand via `pnpm run pipeline` (`tsx src/pipeline/run.ts`) against a `DATABASE_URL` you control. Flags: `--dry-run` (report scope, no writes) and `--limit N` (restrict to the first N discovered repos).
