...
The AI assessment (`src/pipeline/enrich.ts`) is fully automated — there is no hand-authored override path (the old `tracker.html` had an `ASSESS` block for that; it's gone, along with `tracker.html` itself). To avoid re-calling the LLM on every repo every run, each repo's inputs (readme + commit messages + issue/PR titles and state) are hashed into `input_hash`; enrichment only calls the LLM when that hash changes since the last assessment. Results are appended to `repo_assessments` (never overwritten) — "current" is just the latest row per repo by `created_at`. `repos.assessment_source` (`'auto'` | `'manual'`) mirrors `ignore_source`: a repo marked `'manual'` is never re-enriched automatically.

## Gotchas

- **The `superpowers` skill plugin won't show up in `ListPlugins`/`SearchPlugins`.** `.claude/settings.json` registers `obra/superpowers` as a git-sourced marketplace (`extraKnownMarketplaces`, name `superpowers-dev` — matches the marketplace's own declared name, not an arbitrary alias) and enables its plugin (`enabledPlugins`: `superpowers@superpowers-dev`). Its skills (`superpowers:brainstorming`, `superpowers:writing-plans`, `superpowers:systematic-debugging`, etc. — the source of `docs/superpowers/{specs,plans}/`) load automatically at session start and are invoked via the `Skill` tool. `ListPlugins`/`SearchPlugins` only search the org's claude.ai catalog, a separate system that doesn't include repo-declared marketplace plugins — check the `Skill` tool's own listing instead, not those.
