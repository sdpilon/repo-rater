...
describe("renderReadme", () => {
  ...
  it("renders a level-1 header as an <h1> tag", () => {
    expect(renderReadme("# Hello", "octocat/hello-world")).toContain(
      "<h1>Hello</h1>",
    );
  });
  ...
  it("renders GFM strikethrough as <del>", () => {
    expect(renderReadme("~~gone~~", "octocat/hello-world")).toContain(
      "<del>gone</del>",
    );
  });
  ...
  it("does not resolve a backslash-prefixed href to a third-party origin", () => {
    ...
    expect(html).toContain(
      'href="https://github.com/octocat/hello-world/blob/HEAD/evil.com/p"',
    );
  });
